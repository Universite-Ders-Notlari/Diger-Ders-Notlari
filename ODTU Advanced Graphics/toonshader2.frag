...
varying vec3 normal; //normal per-pixel is provided (by rasterizer)
void main()                //better result thanks to better normals
{ 
   float intensity; 
   vec4 color; 
   intensity = dot(lightDir,normalize(normal)); //compute w/ normal
