...
void OnInit()
{
	...
	XMVECTOR Eye = XMVectorSet(0.0, 4.0, -9.0, 0.0);
	...
	m_constantBufferData.mLightPos		= XMFLOAT4(0,  10,  -12, 0);
	m_constantBufferData.mEyePos		= XMFLOAT4(0,  4,  -9, 0);
}
...
{
	//
	// Constant Buffer Settings for Big Cube
	//
	...
	//
	// Constant Buffer Settings for Small Cube
	//

	XMMATRIX Rot30  = XMMatrixRotationY(XM_PI / 6);           // 30 CW
	XMMATRIX Rot45  = XMMatrixRotationY(XM_PI / 4);           // 45 CW
	XMMATRIX Rot60  = XMMatrixRotationY(XM_PI / 3);           // 60 CW
	XMMATRIX Trans  = XMMatrixTranslation(5.0f, 0.0f, 0.0f);
	XMMATRIX Scale  = XMMatrixScaling(0.5f, 0.5f, 0.5f);
	
	//2017-2018 Butunleme Sorusu : Farklı olan hangisi?
	//g_World = Rot30 * Scale * Rot60 * Scale * Trans;
	//g_World = Trans * Scale * Rot30 * Scale * Rot60;
	//g_World = Scale * Rot30 * Scale * Rot60 * Trans;
	//g_World = Trans * Rot30 * Scale * Rot60 * Scale;
	g_World = Scale * Trans * Rot30 * Scale * Rot60; // *
...
HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow)
{
	...
	wcex.lpszClassName	= L"D3D12TextureMapping";
	...
	m_hwnd = CreateWindow(
				L"D3D12TextureMapping", L"DirectX12 > Texture Mapping",
