//
//	DirectX12 > Rendering Triangle : Vertex Buffer	
//
...
ComPtr<ID3D12Resource>				m_vertexBuffer_2;
D3D12_VERTEX_BUFFER_VIEW			m_vertexBufferView_2;
...
void OnInit()
{
	...
	//// Create the vertex buffer _2.
	//{
	//	// Define the geometry for a triangle.
	//	Vertex triangleVertices[] =
	//	{
	//		{ XMFLOAT3( 4.0f,  1.0f,  1.0f), XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f) },
	//		{ XMFLOAT3( 4.0f, -1.0f,  1.0f), XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f) },
	...
	//		D3D12_RESOURCE_STATE_GENERIC_READ,
	//		nullptr,
	//		IID_PPV_ARGS(&m_vertexBuffer_2)));

	//	// Copy the triangle data to the vertex buffer.
	//	UINT8* pVertexDataBegin;
	//	CD3DX12_RANGE readRange(0, 0);		// We do not intend to read from this resource on the CPU.
	//	ThrowIfFailed(m_vertexBuffer_2->Map(0, &readRange, reinterpret_cast<void**>(&pVertexDataBegin)));
	//	memcpy(pVertexDataBegin, triangleVertices, sizeof(triangleVertices));
	//	m_vertexBuffer_2->Unmap(0, nullptr);

	//	// Initialize the vertex buffer view.
	//	m_vertexBufferView_2.BufferLocation = m_vertexBuffer_2->GetGPUVirtualAddress();
	//	m_vertexBufferView_2.StrideInBytes  = sizeof(Vertex);
	//	m_vertexBufferView_2.SizeInBytes    = vertexBufferSize;
	//}
...
{
	//
	// Constant Buffer Settings for Triangle
	//
	...
	memcpy(m_pCbvDataBegin_ChangesEveryFrame + 0 * 256, &m_constantBufferData_ChangesEveryFrame, sizeof(m_constantBufferData_ChangesEveryFrame));

	g_World = XMMatrixTranslation(3, 0, 0) * XMMatrixRotationY(-rotation);
	m_constantBufferData_ChangesEveryFrame.mWorld = XMMatrixTranspose(g_World);
	m_constantBufferData_ChangesEveryFrame.color = XMFLOAT4(1, 1, 1, 1);
	memcpy(m_pCbvDataBegin_ChangesEveryFrame + 1 * 256, &m_constantBufferData_ChangesEveryFrame, sizeof(m_constantBufferData_ChangesEveryFrame));
	...
	g_World = XMMatrixTranslation(0, 3, 0) * XMMatrixRotationX(rotation);
	m_constantBufferData_ChangesEveryFrame.mWorld = XMMatrixTranspose(g_World);
	...
	g_World = XMMatrixTranslation(0, -3, 0) * XMMatrixRotationX(-rotation);
	m_constantBufferData_ChangesEveryFrame.mWorld = XMMatrixTranspose(g_World);
	m_constantBufferData_ChangesEveryFrame.color = XMFLOAT4(1, 1, 0, 1);
...
void OnRender()
{
	...
	m_commandList->SetGraphicsRootConstantBufferView(0, m_constantBuffer_ChangesEveryFrame->GetGPUVirtualAddress() + 0 * 256);
	m_commandList->SetGraphicsRootConstantBufferView(1, m_constantBuffer_NeverChanges->GetGPUVirtualAddress() );

	m_commandList->IASetVertexBuffers(0, 1, &m_vertexBufferView);
	m_commandList->DrawInstanced(6, 1, 0, 0);

	m_commandList->SetGraphicsRootConstantBufferView(0, m_constantBuffer_ChangesEveryFrame->GetGPUVirtualAddress() + 1 * 256);
	m_commandList->DrawInstanced(6, 1, 0, 0);

	m_commandList->SetGraphicsRootConstantBufferView(0, m_constantBuffer_ChangesEveryFrame->GetGPUVirtualAddress() + 2 * 256);
	m_commandList->DrawInstanced(6, 1, 0, 0);

	m_commandList->SetGraphicsRootConstantBufferView(0, m_constantBuffer_ChangesEveryFrame->GetGPUVirtualAddress() + 3 * 256);
	m_commandList->DrawInstanced(6, 1, 0, 0);

	m_commandList->SetGraphicsRootConstantBufferView(0, m_constantBuffer_ChangesEveryFrame->GetGPUVirtualAddress() + 4 * 256);
	m_commandList->DrawInstanced(6, 1, 0, 0);

	//m_commandList->IASetVertexBuffers(0, 1, &m_vertexBufferView_2);
	//m_commandList->DrawInstanced(6, 1, 0, 0);
...
HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow)
{
	...
	wcex.lpszClassName	= "D3D12RenderingTriangle";
	...
	m_hwnd = CreateWindow(
				"D3D12RenderingTriangle", "DirectX12 > Rendering Triangle : Vertex Buffer",
