...
namespace RayTracer 
{
	...
	public:
		...
		void TextureMapping(Vertex Ro, Vertex Rd, Shape* S)
		{
			switch (S->Tex)
			{
				case 1:
				{
					...
					Vertex V0_iPoint = iPoint - S->V0;
					Vertex V0_V1 = (S->V1 - S->V0).Normalize();
					float u = (V0_iPoint * V0_V1) / (S->V1 - S->V0).Length();

					Vertex V0_V2 = (S->V2 - S->V0).Normalize();
					float v = (V0_iPoint * V0_V2) / (S->V2 - S->V0).Length();

					int iks = (int)(u * 680);
					int ye = (int)(v * 1024);

					iks = iks > 679 ? 679 : iks;        iks = iks < 0 ? 0 : iks;
					ye = ye >  1023 ? 1023 : ye;       ye = ye < 0 ? 0 : ye;

					S->ShapeColor = myBitmap_1.GetPixel(iks, ye);
				}
				break;
				...
				case 2:
				{
					...
					Vertex V0_iPoint = iPoint - S->V0;
					Vertex V0_V1 = (S->V1 - S->V0).Normalize();
					float u = 1 - (V0_iPoint * V0_V1) / (S->V1 - S->V0).Length();

					Vertex V0_V2 = (S->V2 - S->V0).Normalize();
					float v = 1 - (V0_iPoint * V0_V2) / (S->V2 - S->V0).Length();
				...
				case 3:
				{
					...
					iks = iks > 249 ? 249 : iks;      iks = iks < 0 ? 0 : iks;
					ye = ye >  166 ? 166 : ye;       ye = ye < 0 ? 0 : ye;

					S->ShapeColor = myBitmap_2.GetPixel(iks, ye);
				}
				break;
				...
				case 5:
				{
					...
					iks = iks > 2799 ? 2799 : iks;      iks = iks < 0 ? 0 : iks;
					ye  = ye  > 4199 ? 4199 :  ye;       ye =  ye < 0 ? 0 : ye;

					S->ShapeColor = myBitmap_3.GetPixel(iks, ye);
				}
				break;
				...
				case 7:
				{
					...
					iks = iks > 1349 ? 1349 : iks;      iks = iks < 0 ? 0 : iks;
					ye = ye  > 3599 ? 3599 : ye;       ye = ye < 0 ? 0 : ye;

					S->ShapeColor = myBitmap_4.GetPixel(iks, ye);
				}
				break;
		...
		Vertex rotateLeft(Vertex P, Vertex camera)
		{
			...
			P.X =  P.X * 0.0F - P.Z * 1.0F;
			P.Z = tmpX * 1.0F + P.Z * 0.0F;
		...
		Color TraceRay(Vertex Ro, Vertex Rd, Shape* Shapes[], Vertex camera, int depth, Shape* prevShape)
		{
			...
			if (Intersections.size() > 0)
			{
				...
				//if (TestShadow(Shapes, intersectionPoint))
		...
		private: System::Void RayTracer_KeyPress(System::Object^  sender, System::Windows::Forms::KeyPressEventArgs^  e)
		{
			...
			switch (e->KeyChar)
			{
				case 'w':
				{
					...
					String^ message =
						"Camera = ( " + camera.X.ToString() + ", " + camera.Y.ToString() + ", " + camera.Z.ToString() + " ) \n" +
