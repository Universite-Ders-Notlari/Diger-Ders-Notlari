//
//	DirectX12 > Rendering MAYA Models	
//
...
Vertex* STL_Loader(char* filename, int* verticesCount);
...
void OnInit()
{
	...
	// Create the vertex buffer ofr MAYA Model.
	{	
		// Read .obj file and fill vertex buffer 
		Vertex* vertices_Model = Obj_Loader("Media/F_35.obj", &vertexCount_Model);	
		//Vertex* vertices_Model = STL_Loader("Media/F_35.stl", &vertexCount_Model);

		const UINT vertexBufferSize = vertexCount_Model * sizeof(Vertex);
		...
		memcpy(pVertexDataBegin, &vertices_Model[0], vertexBufferSize);
	...
	XMVECTOR Eye = XMVectorSet(0.0f, 5.0, -15.0, 0.0);
	XMVECTOR At  = XMVectorSet(0.0f, 0.0,   1.0, 0.0);
	XMVECTOR Up  = XMVectorSet(0.0f, 1.0,   0.0, 0.0);
	...
	m_constantBufferData.mEyePos		= XMFLOAT4(  0,  5, -15, 0);
}
...
{
	//
	// Constant Buffer Settings for MAYA Model
	//

	rotation += 0.01;
	m_constantBufferData.mWorld = XMMatrixTranspose(XMMatrixRotationY(rotation) * g_World);
...
void OnRender()
{
	...
	m_commandList->DrawInstanced(vertexCount_Model, 1, 0, 0);
...
HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow)
{
	...
	wcex.lpszClassName	= "D3D12RenderingMAYAModels";
	...
	m_hwnd = CreateWindow(
				"D3D12RenderingMAYAModels", "DirectX12 > Rendering MAYA Models",
...
Vertex* Obj_Loader(char* filename, int* verticesCount)
{
	...
	*verticesCount = faceCount * 3;
	...
	//FILE* tex = fopen("texture.txt", "w");
	//k = 0;
	//for (int i = 0; i<faceIndex; i++)
	//{
	//	fprintf(tex, "verticesModel[%d].texture.x = %8.6f \t  verticesModel[%d].texture.y = %8.6f  \n", k, verticesModel[k].texture.x, k, verticesModel[k].texture.y);
	//	k++;
	//	fprintf(tex, "verticesModel[%d].texture.x = %8.6f \t  verticesModel[%d].texture.y = %8.6f  \n", k, verticesModel[k].texture.x, k, verticesModel[k].texture.y);
	//	k++;
	//	fprintf(tex, "verticesModel[%d].texture.x = %8.6f \t  verticesModel[%d].texture.y = %8.6f  \n", k, verticesModel[k].texture.x, k, verticesModel[k].texture.y);
	...
	*verticesCount = vertexCount;
	...
	while (!fin.eof())
	{
		...
		if (input == 'f')
		{
			...
			verticesModel[v].normal.x = x;
			verticesModel[v].normal.y = y;
			verticesModel[v].normal.z = -1.0 * z;

			verticesModel[v + 1].normal.x = x;
			verticesModel[v + 1].normal.y = y;
			verticesModel[v + 1].normal.z = -1.0 * z;

			verticesModel[v + 2].normal.x = x;
			verticesModel[v + 2].normal.y = y;
			verticesModel[v + 2].normal.z = -1.0 * z;
		}
		...
		if (input == 'v')
		{
			...
			verticesModel[v].position.x = x;
			verticesModel[v].position.y = y;
			verticesModel[v].position.z = -1.0 * z;

			verticesModel[v].texture.x = 0;
			verticesModel[v].texture.y = 0;
		...
		// Convert vertex orders from CCW to CW (Clock Wise)
		if ((v % 3) == 0)
		{
			float tmpX, tmpY, tmpZ;

			tmpX = verticesModel[v - 1].position.x;
			tmpY = verticesModel[v - 1].position.y;
			tmpZ = verticesModel[v - 1].position.z;

			verticesModel[v - 1].position.x = verticesModel[v - 2].position.x;
			verticesModel[v - 1].position.y = verticesModel[v - 2].position.y;
			verticesModel[v - 1].position.z = verticesModel[v - 2].position.z;

			verticesModel[v - 2].position.x = tmpX;
			verticesModel[v - 2].position.y = tmpY;
			verticesModel[v - 2].position.z = tmpZ;
...
void OnMouseMove(WPARAM btnState, int x, int y)
{
	...
	else if ((btnState & MK_RBUTTON) != 0)
	{
		...
		// Restrict the radius.
		mRadius = Clamp(mRadius, 10.0f, 40.0f);
	}
