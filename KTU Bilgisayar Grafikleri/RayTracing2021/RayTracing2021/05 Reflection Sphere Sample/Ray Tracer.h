...
namespace RayTracer 
{
	...
	public ref class RayTracer : public System::Windows::Forms::Form
	{
		...
		void InitializeComponent(void)
		{
			...
			this->Render->Location = System::Drawing::Point(376, 476);
			...
			this->Text = L"RayTracer : Reflection Sphere Sample";
		...
		Color TraceRay(Vertex Ro, Vertex Rd, Shape* Shapes[], Vertex camera, int depth, Shape* prevShape)
		{
			...
			if (Intersections.size() > 0)
			{
				...
				String^ name;
				if (min_indis == 0) name = "Red Sphere"; else name = "Blue Sphere";
				//if (min_indis == 0) name = "Triangle"; else name = "Sphere";
				String^ message = "t = " + min_distance.ToString() + "\niPoint " + name + "= ( " + iPoint.X.ToString() + ", " + iPoint.Y.ToString() + ", " + iPoint.Z.ToString() + " )";
				MessageBox::Show(message);
		...
		private: System::Void Render_Click(System::Object^  sender, System::EventArgs^  e) 
		{
			...
			// Işın Kırmızı renkli kürenin "dışından" yansıyıp Mavi renkli küreyle kesişiyor
			// https://ceng2.ktu.edu.tr/~cakir/files/grafikler/Grafikler1.VizeCevap2013.pdf
			Sphere   S1(Vertex(80,  15, 0), 10, 0.0F, 0.5F, 0.0F, 0.5F, Color::Red);
			Sphere   S2(Vertex(72, 121, 0), 35, 0.3F, 0.7F, 0.0F, 0.0F, Color::Blue);

			Shape* Shapes[2] = { &S1, &S2 };

			// Kürenin "içinden" yansımaya ait iki örnek
			// https://ceng2.ktu.edu.tr/~cakir/files/grafikler/Grafikler1.VizeCevap2012.pdf : Işın Mavi renkli kürenin içinden yansıyıp Kırmızı renkli üçgenle kesişiyor
			// https://ceng2.ktu.edu.tr/~cakir/files/grafikler/Grafikler1.VizeCevap2017.pdf : Işın kürenin içinden iki kere yansıyıp üçgenle kesişiyor
			//Triangle T1(Vertex(0, 20, -34), Vertex(-10, -4, -52), Vertex(10, -4, -52), 0.3F, 0.7F, 0.0F, 0.0F, Color::Red);
			//Sphere   S1(Vertex(0, 0, 0), 100, 0.0F, 0.5F, 0.0F, 0.5F, Color::Blue);

			//Shape* Shapes[2] = { &T1, &S1 };

			Vertex camera = Vertex(0, 0, 0);
			//Vertex camera = Vertex(0, -80, -25);

			Vertex Rd(0.96F, 0.28F, 0);
			//Vertex Rd(0, 0.8F, 0.6F);
