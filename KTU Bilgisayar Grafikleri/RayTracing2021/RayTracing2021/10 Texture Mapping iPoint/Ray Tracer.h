...
namespace RayTracer 
{
	...
	public ref class RayTracer : public System::Windows::Forms::Form
	{
		...
		void InitializeComponent(void)
		{
			...
			this->Text = L"RayTracer : Texture Mapping iPoint";
		...
		void TextureMapping(Shape* S, Vertex iPoint)
		{
			switch (S->Tex)
			{
				case 1:
				{
					...
					// II. COZUM	(Abs hangilerinde gereksiz?)
					//float u = Math::Abs(iPoint.X - S->V0.X) / (S->V1 - S->V0).Length();
					//float v = Math::Abs(iPoint.Z - S->V0.Z) / (S->V2 - S->V0).Length();
				...
				case 2:
				{
					...
					// II. COZUM
					//float u = 1 - Math::Abs(iPoint.X - S->V0.X) / (S->V1 - S->V0).Length();
					//float v = 1 - Math::Abs(iPoint.Z - S->V0.Z) / (S->V2 - S->V0).Length();
				...
				case 3:
				{
					...
					// II. COZUM
					//float u = Math::Abs(iPoint.X - S->V0.X) / (S->V1 - S->V0).Length();
					//float v = Math::Abs(iPoint.Y - S->V0.Y) / (S->V2 - S->V0).Length();
				...
				case 4:
				{
					...
					// II. COZUM
					//float u = 1 - Math::Abs(iPoint.X - S->V0.X) / (S->V1 - S->V0).Length();
					//float v = 1 - Math::Abs(iPoint.Y - S->V0.Y) / (S->V2 - S->V0).Length();
				...
				{
					// I. COZUM
					float u = (iPoint.Z - S->V0.Z) / (S->V1.Z - S->V0.Z);
					float v = (iPoint.Y - S->V0.Y) / (S->V2.Y - S->V0.Y);

					// II. COZUM
					//float u = Math::Abs(iPoint.Z - S->V0.Z) / (S->V1 - S->V0).Length();
					//float v = Math::Abs(iPoint.Y - S->V0.Y) / (S->V2 - S->V0).Length();
				...
				{
					// I. COZUM
					float u = 1 - (iPoint.Z - S->V0.Z) / (S->V1.Z - S->V0.Z);
					float v = 1 - (iPoint.Y - S->V0.Y) / (S->V2.Y - S->V0.Y);

					// II. COZUM
					//float u = 1 - Math::Abs(iPoint.Z - S->V0.Z) / (S->V1 - S->V0).Length();
					//float v = 1 - Math::Abs(iPoint.Y - S->V0.Y) / (S->V2 - S->V0).Length();
		...
		private: System::Void Render_Click(System::Object^  sender, System::EventArgs^  e) 
		{
			...
			// Mona Lisa (Left Wall)
			Triangle T15(  Vertex(-59,  30,   75),     Vertex(-59,  30, 105),     Vertex(-59, -15,  75),   0.3, 0.7, 0.0, 0.0, 0.0, 0.0, Color::White, 5);
			Triangle T16(  Vertex(-59, -15,  105),     Vertex(-59, -15,  75),     Vertex(-59,  30, 105),   0.3, 0.7, 0.0, 0.0, 0.0, 0.0, Color::White, 6);
